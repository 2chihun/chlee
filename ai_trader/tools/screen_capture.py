...
사용법:
    python tools/screen_capture.py                     # Edge 창 3초 간격 캡처
    python tools/screen_capture.py --interval 1        # 1초 간격
    python tools/screen_capture.py --count 50          # 50장만 캡처
    python tools/screen_capture.py --fullscreen        # 전체 화면 캡처
    python tools/screen_capture.py --ocr               # OCR 텍스트 추출 포함
    python tools/screen_capture.py --list-windows      # 캡처 가능한 창 목록
    python tools/screen_capture.py --title "제목"      # 특정 창 제목으로 캡처

    # 📖 책 읽기 모드: 우측 > 버튼 랜덤 클릭 + 화면 변경 시 자동 캡처
    #   - 되돌아감 자동 감지 + 중단 / 50페이지마다 10초 휴식
    #   - 전역 단축키: Ctrl+Alt+Shift+B(시작) / Ctrl+Alt+Shift+E(종료)
    python tools/screen_capture.py --book --title "교과서" --monitor 2 --count 290
    python tools/screen_capture.py --book --title "교과서" --monitor 2 --count 290 --rest-interval 50 --rest-seconds 10

    # 🔍 캡처 폴더 중복 이미지 제거 (단독 실행)
    python tools/screen_capture.py --dedup --output captures

    # 📋 캡처 결과 검증 (누락/이상치 확인)
    python tools/screen_capture.py --verify --output captures --title "교과서" --expected 290
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="엣지 브라우저(또는 특정 창) 자동 주기 캡처 도구"
    )
